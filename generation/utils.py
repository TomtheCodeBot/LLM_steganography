...
class GenerationMixin:
    ...
    def decode_stega(
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            # 11. run greedy search
            return self.decode_greedy_search(
                input_ids,
        ...
        raise Exception("Can't decode current encoding strategy.")
    @torch.no_grad()
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if self.stega_type == "binary":
                ...
                probs_array = probs_array.cpu().numpy()
                coding = HuffmanCoding()
                coding.make_heap_from_array(probs_array[0])
                coding.merge_nodes()
                root = coding.make_codes()
                length = len(self.bit_stream)

                bit_stream+=coding.codes[rank]
                next_tokens = torch.tensor([output_ids[token_idx]]).to(self.model.device)
                token_idx+=1
                ...
                new_bits = list(map(str,new_bits))
                bit_stream += "".join(new_bits)
                ...
                raise Exception("Steganography type not found.")
            # finished sentences should have their next token be a padding token
            if eos_token_id is not None:
                if pad_token_id is None:
                    raise ValueError("If `eos_token_id` is defined, make sure that `pad_token_id` is defined.")
            ...
            if self.stega_type is None:
                ...
                if chunk>=len(self.bit_stream):
                    ...
                    while root.token is None:
                        if chunk >= length or self.bit_stream[chunk] == "0":
                            root = root.left
                        else:
                            root = root.right
                        chunk += 1
